 ...
 * Implements a logarithmic segregated fit topology to minimize internal 
 * fragmentation while maintaining O(1) mathematical resolution.
 */
...
/** @brief Minimum allocation size to safely store lock-free next pointers. */
...
static LZ_ALWAYS_INLINE uint32_t lz_size_to_class(size_t size) {
    ...
    /* Linear mapping for sizes under 128 bytes (16B steps) */
    if (LZ_LIKELY(s < LZ_SMALL_LINEAR_MAX)) {
        return (uint32_t)(s >> 4);
    }

    /* Logarithmic mapping for sizes >= 128 bytes */
    uint32_t msb = lz_fls(s);
    uint32_t shift = msb - 2;
    uint32_t base = (msb - 7) << 2;       
